...
//in cannot be out
{
    ...
    element_t temp;
	element_ptr in0, in1, v0, v1, t0, t1;
...
static void cc_tatepower(element_ptr out, element_ptr in, pairing_t pairing)
{
    ...
    if (p->k == 6) {
	element_t e0, e2, e3;
	element_ptr e0re, e0im, e0re0, e0im0;
	element_t *inre, *inim;
	...
	e0re = fi_re(e0);
	e0im = fi_im(e0);
	e0re0 = ((element_t *) e0re->data)[0];
	e0im0 = ((element_t *) e0im->data)[0];
	inre = fi_re(in)->data;
	inim = fi_im(in)->data;
...
static void d_pairing_pp_apply(element_ptr out, element_ptr in2, pairing_pp_t p)
{
    ...
    m = mpz_sizeinbase(q, 2) - 2; //Moved by Michael Cheng
...
//MNT Curves
//Embeding degree 6
void d_param_input(pairing_t pairing)
{
	d_param_t dp;
    int d;
	...
	d_param_init(dp);
	mpz_set_str(dp->q, "625852803282871856053922297323874661378036491717", 0);
	mpz_set_str(dp->n, "625852803282871856053923088432465995634661283063", 0);
	mpz_set_str(dp->h, "3", 0);
	mpz_set_str(dp->r, "208617601094290618684641029477488665211553761021", 0);
	mpz_set_str(dp->a, "581595782028432961150765424293919699975513269268", 0);
	mpz_set_str(dp->b, "517921465817243828776542439081147840953753552322", 0);
	dp->k=6;
	mpz_set_str(dp->nk, "", 0);
	mpz_set_str(dp->hk, "1380801711862212484403205699005242141541629761433899149236405232528956996854655261075303661691995273080620762287276051361446528504633283152278831183711301329765591450680250000592437612973269056", 0);
	mpz_set_str(dp->nqr, "431211441436589568382088865288592347194866189652", 0);

    d = dp->k / 2;
    dp->coeff = pbc_realloc(dp->coeff, sizeof(mpz_t) * d);
	mpz_init(dp->coeff[0]);
	mpz_set_str(dp->coeff[0], "472731500571015189154958232321864199355792223347", 0);
	mpz_init(dp->coeff[1]);
	mpz_set_str(dp->coeff[1], "352243926696145937581894994871017455453604730246", 0);
	mpz_init(dp->coeff[2]);
	mpz_set_str(dp->coeff[2], "289113341693870057212775990719504267185772707305", 0);
		
	pairing_init_d_param(pairing, dp);
	d_param_clear(dp);
