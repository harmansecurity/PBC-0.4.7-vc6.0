...
static void fp_mul_si(element_ptr c, element_ptr a, signed long int op)
{
	...
	mp_limb_t tmp[MAX_LIMBS + 1];	//By Michael Cheng
	mp_limb_t qp[2];
	...
	mp_limb_t tmp[t + 1];
	mp_limb_t qp[2];
#endif //Modified by Michael Cheng
...
static int fp_sgn_even(element_ptr a)
{
    ...
    fp_field_data_ptr p = a->field->data;
    dataptr ad = a->data;
//    mp_limb_t sum[p->limbs]; Modified by Michael Cheng
	mp_limb_t sum[MAX_LIMBS];
    if (!ad->flag) return 0;


    carry = mpn_add_n(sum, ad->d, ad->d, p->limbs);
    if (carry) return 1;
